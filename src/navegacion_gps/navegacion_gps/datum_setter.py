...
class DatumSetterNode(Node):
    def __init__(self) -> None:
        ...
        self._get_datum_srv = self.create_service(
            GetDatum,
    ...
    def _build_status_message(
        ...
        quality_msg = (
            ""
            if gps_is_rtk
            else " Warning: datum was applied without RTK quality."
        )
    ...
    def _on_set_datum(
        ...
        with self._lock:
            ...
            current_fix = self._last_gps_fix

        if use_current_gps:
            if current_fix is None:
                response.ok = False
                response.error = "no current GPS sample available"
                response.status_message = ""
                response.already_set_before = bool(self.already_set)
        ...
        ok_apply, apply_error, status_message, already_before = self._apply_datum(
            lat=float(lat),
            lon=float(lon),
            source="service_current_gps" if use_current_gps else "service_manual_coords",
            gps_is_rtk=bool(gps_is_rtk),
            used_current_gps=bool(use_current_gps),
        )
        response.ok = bool(ok_apply)
        response.error = "" if ok_apply else str(apply_error)
        response.status_message = str(status_message) if ok_apply else ""
        response.already_set_before = bool(already_before)
        response.used_current_gps = bool(use_current_gps)
        response.gps_is_rtk = bool(gps_is_rtk)
        response.applied_lat = float(lat)
        response.applied_lon = float(lon)
        return response
    ...
    def _on_get_datum(
        ...
        with self._lock:
            already_set = bool(self.already_set)
            gps_is_rtk = bool(self._combined_rtk_locked())
            current_gps = self._last_gps_fix
            ...
            last_set_with_rtk = bool(self._last_set_with_rtk)

        response.ok = True
        response.error = ""
        response.already_set = already_set
        response.has_current_gps = current_gps is not None
        response.gps_is_rtk = gps_is_rtk
        if current_gps is None:
            response.current_gps_lat = float("nan")
            response.current_gps_lon = float("nan")
        else:
            response.current_gps_lat = float(current_gps[0])
            response.current_gps_lon = float(current_gps[1])
